...
export default function ReviewPage() {
  ...
  return (
    <div><p className="mb-3 text-xs uppercase tracking-widest text-[#B89A4E]">Review</p><h1 className="font-serif text-4xl text-[#1C1917]">One basket. <span className="italic text-[#B89A4E]">One tap.</span></h1><p className="mt-3 text-xs uppercase tracking-widest text-[#B89A4E]">The Slow Ganges · 14 days</p>
      ...
      <div className="mt-6 flex justify-between"><button onClick={() => router.back()} className="rounded-full border border-[#1C1917] px-6 py-3 text-sm">Adjust</button><button onClick={() => router.push('/build/confirm')} className="rounded-full bg-[#B89A4E] px-6 py-3 text-sm text-white">Confirm in one tap -&gt;</button></div>
    </div>
  )
}
