...
export default function BuildCounselPage() {
  return (
    <Suspense fallback={<div className="text-sm text-[#9C9589]">Loading counsel...</div>}>
      <BuildCounselContent />
    </Suspense>
  )
}
...
function BuildCounselContent() {
  ...
  return (
    <div>
      ...
      <div className="mt-8 space-y-4">{messages.map((message, index) => <div key={index} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-xs sm:max-w-md rounded-xl px-4 py-3 ${message.sender === 'user' ? 'border border-[#E8E3D9] bg-white' : 'bg-[#F5F0E8]'}`}><p className="text-sm text-[#1C1917]">{message.content}</p></div></div>)}</div>
      ...
      <button onClick={() => router.push(`/build/review?type=${tripType}&journey=${journey}`)} className="mt-6 w-full rounded-full bg-[#1C1917] py-3 text-sm text-white">See the basket -&gt;</button>
    </div>
  )
}
