...
export default function AdminCounsel() {
  ...
  useEffect(() => {
    if (!selected) return
    supabase.from('messages').update({ is_read: true }).eq('booking_id', selected).eq('sender_type', 'user').then(() => {
      setMessages((current) => current.map((message) => message.booking_id === selected && message.sender_type === 'user' ? { ...message, is_read: true } : message))
    })
  }, [selected, supabase])
  ...
  return (
    <div className="flex flex-col md:flex-row h-full bg-[#FAFAF8]">
      <aside className={`${selected ? 'hidden md:block' : 'block'} w-full md:w-80 shrink-0 border-b md:border-b-0 md:border-r border-[#E8E3D9] bg-white overflow-y-auto`}>
