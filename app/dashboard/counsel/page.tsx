...
type ChatMessage = { sender: 'soma' | 'user'; content: string; bookingId?: string | null }
...
export default function SomaPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: auth }) => {
      if (!auth.user) return
      const { data: booking } = await supabase.from('bookings').select('id').eq('user_id', auth.user.id).in('status', ['on_path', 'confirmed', 'new']).order('created_at', { ascending: false }).limit(1).maybeSingle()
      setActiveBookingId(booking?.id ?? null)
      const query = supabase.from('messages').select('sender_type, content, booking_id').order('created_at').limit(12)
      const { data } = booking?.id ? await query.eq('booking_id', booking.id) : await query
      if (data?.length) setMessages((current) => [...current, ...data.map((row) => ({ sender: row.sender_type === 'user' ? 'user' as const : 'soma' as const, content: row.content, bookingId: row.booking_id }))])
    })
  }, [supabase])
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <footer className="border-t border-[#E8E3D9] bg-white p-4"><div className="flex items-center gap-3 rounded-full border border-[#E8E3D9] px-4 py-3"><Mic size={16} className="text-[#9C9589]" /><input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') send() }} className="flex-1 outline-none" placeholder="Ask Soma anything..." /><button onClick={() => send()} className="text-[#B89A4E]"><Send size={16} /></button></div></footer>
    </div>
  )
}
